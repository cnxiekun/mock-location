...
class SuspendedPhaseTest {
    ...
    fun `push duration elapsed transitions from pushing to paused`() {
        ...
        val next =
            advanceSuspendedPhase(start, now = pushDur, enabled = true, mode = MockMode.TELEPORT, random = Random(1))
    ...
    fun `paused phase resets to pushing when mode is ROUTE_REPLAY`() {
        val start = paused(startMs = 0L)
        val next =
            advanceSuspendedPhase(start, now = 0L, enabled = true, mode = MockMode.ROUTE_REPLAY, random = Random(1))
